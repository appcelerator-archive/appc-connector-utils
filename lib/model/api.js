...
function createFromDescription(modelDescriptions, options) {
	return modelDescriptions.keys().map(function(modelName) {
		const description = modelDescriptions[modelName];
		Validator.validate(description, modelContract);
		return Arrow.createModel(modelName, description);
	});
}
