...
/**
 * Probably specific contract for Swagger connector
 */
...
const modelFieldsContract = Joi.object().pattern(/.*/, Joi.object({
	 ...
	 */
	type: Joi.string(), //this is required according to the docs ?!
	...
	// misssing from the documentation
	originalType: Joi.string(),
	optional: Joi.boolean(), // duplicated with required; may be remove this one since it is not in the docs
	foreignKeyName: Joi.string()
}));

const modelContract = Joi.object({	
	/**
	 * An object that represents the model’s schema define as key-value pairs. The key is the name of the field and the value is the fields object. See modelFieldsContract for details.
	 */
	fields: modelFieldsContract, //required ?!
	/**
	...
	// Documented but never used so far
	/**
	...
	// These are not in the documentations
	name: Joi.string().required(),	
	generated: Joi.boolean().required(), //optional ?!
